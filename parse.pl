...
my $login = "Lenny@redteam-cafe.com";
my $password = "&72L6*HMe4b5LjA";
...
sub get_from_moex {
    my $ticker = shift;
    my $url = 'iss/securities/'.$ticker.'.xml';
    ...
    if ( $data->at('data#description > rows')->children->size ) {
		...
		$url = 'iss/engines/stock/markets/shares/boards/'.$res->{primary_boardid}.'/securities.xml?securities='.$ticker;
...
my @links = $driver->find_elements("//div[\@data-qa-file='FavoritesWidgetPure']//a");
